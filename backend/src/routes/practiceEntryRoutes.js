...
router.get("/", authMiddleware, async (req, res) => {
  ...
  try {
    const [entries] = await pool.query(
      "SELECT practice_entries.id, practice_date, duration_minutes, category, name, piece_title, notes FROM practice_entries JOIN instruments ON practice_entries.instrument_id = instruments.id WHERE practice_entries.user_id = ? AND practice_date >= CURDATE() - INTERVAL 1 YEAR ORDER BY practice_date DESC",
      [userId],
    );
